...
| **`src/maturity.ts:270` scores `live_browser_verification` as `met` from a filename match** | changing a score is a product change. Now stated in the module's own header so nobody trusts it by accident. |
