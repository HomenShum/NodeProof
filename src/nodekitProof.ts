 ...
 */
export const NODEKIT_PROOF_RECEIPT_SCHEMA = "nodekit.proof-receipt/v1" as const;
export const NODEKIT_COMPILED_DEFINITION_SCHEMA = "nodeagent.resolved/v1" as const;
export const NODEKIT_DISCOVERY_SCHEMA = "nodeagent.discovery/v1" as const;
 ...
 */
export function verifyNodekitProofBinding(options: VerifyNodekitProofBindingOptions): NodekitProofBindingVerification {
  const root = resolve(options.root);
  const minimumLevel = options.minimumLevel ?? "local-ready";
  const errors: string[] = [];
  const gateReceipts: NodekitProofGateReceipt[] = [];
  ...
  let identity: NodekitProofApplicationIdentity | undefined;
  if (compiled && configHash && discovery && discoveryPath) {
    ...
    identity = {
      ...
      discoveryDigest: sha256(readFileSync(discoveryPath)),
...
export function formatNodekitProofBindingVerification(result: NodekitProofBindingVerification): string {
  const lines = [
  ...
  for (const receipt of result.gateReceipts) {
    ...
    for (const error of receipt.errors) lines.push(`  - ${error}`);
  }
  if (result.errors.length === 0) lines.push("errors: none");
  else {
    lines.push("errors:");
...
export function runNodekitProofBindingVerify(options: VerifyNodekitProofBindingOptions & {
  json?: boolean;
...
function verifyReleaseProof(args: {
  ...
  if (releaseProof.schemaVersion !== NODEKIT_PROOF_RECEIPT_SCHEMA) {
    errors.push(`NodeKit release proof schemaVersion must be ${NODEKIT_PROOF_RECEIPT_SCHEMA}`);
  }
  if (releaseProof.passed !== true) errors.push("NodeKit release proof must have passed=true");
  ...
  for (const id of ["deterministicDemo", "deterministicEvaluation", "secretFree"]) {
    if (checks[id] !== true) errors.push(`NodeKit release proof checks.${id} must be true`);
  }
  ...
  const proofDirectory = dirname(args.releasePath);
  ...
  const optionalGateSpecs: Array<{
    id: string;
    check: string;
    filename: string;
    expectedSchema?: string;
    requireNodekitSchema?: boolean;
...
function verifyJsonGate(args: {
  ...
  return { id: args.id, path: display, sha256: sha256(readFileSync(file)), ok: errors.length === 0, errors };
}

function verifyReleaseReceiptVerification(
  releaseProof: UnknownRecord,
  ...
  if (verification.passed !== true) errors.push("NodeKit release proof receiptVerification must have passed=true when present");
...
function verifyGateIdentityAndDigest(
  value: UnknownRecord,
  args: { id: string; configHash?: string; candidateCommit?: string },
  errors: string[],
): void {
  ...
  if (value.receiptDigest === undefined) return;
...
function readCompiledDefinition(path: string, errors: string[]): CompiledDefinition | undefined {
  const value = readJsonRecord(path, "NodeKit compiled definition", errors);
  if (!value) return undefined;
  if (value.schemaVersion !== NODEKIT_COMPILED_DEFINITION_SCHEMA) {
    errors.push(`NodeKit compiled definition schemaVersion must be ${NODEKIT_COMPILED_DEFINITION_SCHEMA}`);
  }
  ...
  if (typeof value.configHash !== "string" || typeof value.manifestDigest !== "string" || typeof value.fileCount !== "number") return undefined;
  return {
...
function readConfigHash(path: string, errors: string[]): string | undefined {
...
function readDiscovery(path: string, errors: string[]): { files: DiscoveryFile[] } | undefined {
  const value = readJsonRecord(path, "NodeKit discovery", errors);
  if (!value) return undefined;
  if (value.schemaVersion !== NODEKIT_DISCOVERY_SCHEMA) {
    errors.push(`NodeKit discovery schemaVersion must be ${NODEKIT_DISCOVERY_SCHEMA}`);
  }
...
function verifyManifestDigest(root: string, expected: string, candidateCommit: string, errors: string[]): void {
  const manifestPath = resolveRegularFile(root, "nodeagent.yaml", "NodeKit manifest", errors);
  if (!manifestPath) return;
  if (sha256(readFileSync(manifestPath)) !== expected) {
...
function verifyDiscoveryFiles(root: string, files: DiscoveryFile[], candidateCommit: string, errors: string[]): void {
...
function verifyCandidateFileBytes(
  root: string,
  candidateCommit: string,
  repoPath: string,
  label: string,
  ...
  if (!GIT_SHA_PATTERN.test(candidateCommit)) return;
  try {
    execFileSync("git", ["cat-file", "-e", `${candidateCommit}:${repoPath}`], {
      cwd: root,
  ...
  try {
    ...
    // actually observed.
    execFileSync("git", ["diff", "--quiet", candidateCommit, "--", repoPath], {
      cwd: root,
      stdio: ["ignore", "pipe", "pipe"],
      windowsHide: true,
    });
...
function readCurrentCommit(root: string, errors: string[]): string | undefined {
  try {
...
function resolveRegularFile(rootInput: string, pathInput: string, label: string, errors: string[]): string | undefined {
  ...
  const candidate = resolve(rootInput, pathInput);
...
function readJsonRecord(path: string, label: string, errors: string[]): UnknownRecord | undefined {
  try {
...
function joinRelative(root: string, directory: string, filename: string): string {
  const target = resolve(directory, filename);
  const value = relative(resolve(root), target).split(sep).join("/");
  return value || filename;
}

function displayPath(pathInput: string): string {
  if (!safeRepoRelativePath(pathInput)) return pathInput;
  return pathInput.split(/[\\/]/).join("/");
}

function realPathOrResolved(pathInput: string): string {
  try {
    return resolve(realpathSync(pathInput));
  } catch {
