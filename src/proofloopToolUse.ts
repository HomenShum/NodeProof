...
export const TOOL_USE_CONTRACT_TEMPLATES: Record<string, () => Record<string, unknown>> = {
  "composio-email-triage": () => ({
    $comment:
      "Expected-tool-use contract for a Composio-MCP email-triage agent. Verified against LOCAL PostToolUse capture (.proofloop/tooluse/log.jsonl) -- not Composio server-side attestation.",
    version: 1,
    required: [
      {
...
// ---------------------------------------------------------------------------
// impure CLI runners (cli.ts `tooluse` delegates here)
