...
export function proofloopProviderReceiptPath(root: string, providerId: ProofloopProviderId): string {
  return join(resolve(root), ".proofloop", "setup", "providers", `${providerId}.json`);
}
...
function providerHealthUrl(providerId: ProofloopProviderId, env: NodeJS.ProcessEnv): string | undefined {
  ...
  if (providerId === "nebius") return `${trimTrailingSlash(env.NEBIUS_BASE_URL ?? "https://api.tokenfactory.nebius.com/v1")}/models`;
  return undefined;
}

function providerHeaders(providerId: ProofloopProviderId, env: NodeJS.ProcessEnv): Record<string, string> {
  ...
  if (providerId === "nebius" && env.NEBIUS_API_KEY) headers.Authorization = `Bearer ${env.NEBIUS_API_KEY}`;
  return headers;
}
...
function nextCommands(providerId: ProofloopProviderId, status: ProofloopProviderSetupStatus): string[] {
  ...
  if (status === "ready") return [retry, "npx proofloop gate"];
...
function redactUrl(url: string, env: NodeJS.ProcessEnv): string {
  let value = url;
