...
export const LONG_RUN_RELATIVE_DIR = ".proofloop/longrun";
export const LONG_RUN_PLAN_VERSION = 1;
export const LONG_RUN_DEFAULT_BUDGET_USD = 100;
export const LONG_RUN_DEFAULT_MAX_RETRIES = 2;
export const LONG_RUN_DEFAULT_CONCURRENCY = 1;
export const LONG_RUN_MAX_CONCURRENCY = 16;
export const LONG_RUN_HEARTBEAT_MS = 30_000;
export const LONG_RUN_STALE_LOCK_MS = 5 * 60_000;
/** Redacted stdout/stderr tail stored per attempt_end (chars). */
export const LONG_RUN_SNIPPET_MAX_CHARS = 2_000;
/** Rolling capture cap per stream while the child runs (chars). BOUND_READ. */
export const LONG_RUN_STREAM_CAPTURE_MAX_CHARS = 64 * 1024;
...
export class LongRunPlanError extends Error {}
/** Ledger is unusable (missing / corrupt non-final line). CLI exit 2. */
export class LongRunLedgerError extends Error {}

// ---------------------------------------------------------------------------
// plan schema (fail-closed, mirrors the tooluse contract loader's strictness)
...
function rejectUnknownKeys(record: Record<string, unknown>, allowed: readonly string[], context: string): void {
  for (const key of Object.keys(record)) {
    if (!allowed.includes(key)) {
      throw new LongRunPlanError(
        `${context}: unknown key "${key}" (allowed: ${allowed.join(", ")}). Refusing to guess -- a typo here would silently change what gets executed or spent.`,
      );
    }
  }
}
...
function assertNoEmbeddedSecret(value: string, context: string): void {
  if (SECRET_TOKEN_RE.test(value)) {
    throw new LongRunPlanError(
      `${context}: contains a secret-looking token. Secrets (e.g. OPENROUTER_API_KEY) must be read from the environment by the child command, never stored in the plan.`,
    );
  }
}
...
export function parseLongRunPlan(value: unknown): LongRunPlan {
  if (!isPlainObject(value)) throw new LongRunPlanError("plan must be a JSON object.");
  ...
  if (versionRaw !== undefined && versionRaw !== LONG_RUN_PLAN_VERSION) {
    throw new LongRunPlanError(`plan.version must be exactly ${LONG_RUN_PLAN_VERSION} when present (got ${JSON.stringify(versionRaw)}).`);
  }

  const budgetRaw = ownGet(value, "budgetUsd");
  const budgetUsd = budgetRaw === undefined ? LONG_RUN_DEFAULT_BUDGET_USD : requireNonNegativeNumber(budgetRaw, "plan.budgetUsd");
  if (budgetUsd <= 0) throw new LongRunPlanError("plan.budgetUsd: must be > 0 (a zero budget can never execute anything).");

  const retriesRaw = ownGet(value, "maxRetries");
  const maxRetries =
    retriesRaw === undefined
      ? LONG_RUN_DEFAULT_MAX_RETRIES
      : (() => {
  ...
  const concurrency = concurrencyRaw === undefined ? LONG_RUN_DEFAULT_CONCURRENCY : requirePositiveInt(concurrencyRaw, "plan.concurrency");
  if (concurrency > LONG_RUN_MAX_CONCURRENCY) {
    throw new LongRunPlanError(`plan.concurrency: must be <= ${LONG_RUN_MAX_CONCURRENCY} (BOUND: a runaway pool is how machines melt mid-benchmark).`);
  }
  ...
  const attempts: LongRunAttemptSpec[] = attemptsRaw.map((entry, i) => {
    const context = `plan.attempts[${i}]`;
    if (!isPlainObject(entry)) throw new LongRunPlanError(`${context}: must be an object.`);
    ...
    if (seenIds.has(id)) throw new LongRunPlanError(`${context}.id: duplicate id ${JSON.stringify(id)} (ids key the resume ledger; duplicates would merge histories).`);
    seenIds.add(id);
    ...
    if (noteRaw !== undefined && typeof noteRaw !== "string") throw new LongRunPlanError(`${context}.note: must be a string.`);
    if (typeof noteRaw === "string") assertNoEmbeddedSecret(noteRaw, `${context}.note`);

    return { id, family, taskId, model, command, timeoutMs, estCostUsd, ...(typeof noteRaw === "string" ? { note: noteRaw } : {}) };
  });

  return { version: LONG_RUN_PLAN_VERSION, budgetUsd, maxRetries, concurrency, attempts };
}

// ---------------------------------------------------------------------------
// ledger records (append-only JSONL; first record carries the schema tag)
...
export type LongRunLedgerRecord =
  ...
  | { type: "run_started"; ts: string; pid: number; host: string; mode: "start" | "resume" }
  | { type: "attempt_start"; ts: string; attemptId: string; try: number }
  | {
      type: "attempt_end";
      ts: string;
      attemptId: string;
  ...
  | { type: "budget_exhausted"; ts: string; spentUsd: number; budgetUsd: number; nextAttemptId: string; nextTryEstCostUsd: number }
  | { type: "run_interrupted"; ts: string; signal: string; inFlightAttemptIds: string[] }
  | { type: "run_completed"; ts: string; passed: number; failed: number; spentUsd: number; allPassed: boolean };

const LEDGER_RECORD_TYPES = new Set<string>([
  "run_created",
  "run_started",
  "attempt_start",
  "attempt_end",
 ...
 */
export function appendLongRunLedger(ledgerPath: string, record: LongRunLedgerRecord): void {
 ...
 */
export function readLongRunLedger(ledgerPath: string): LongRunLedgerReadResult {
  ...
  for (const part of raw.split("\n")) {
    if (part.trim().length > 0) lines.push({ text: part, start: offset });
    offset += part.length + 1; // the "\n" split on (harmlessly overshoots at EOF)
  }
  const records: LongRunLedgerRecord[] = [];
...
export function deriveLongRunProgress(plan: LongRunPlan, records: LongRunLedgerRecord[]): LongRunProgress {
  const maxTries = plan.maxRetries + 1;
  const startsById = new Map<string, Extract<LongRunLedgerRecord, { type: "attempt_start" }>[]>();
  const endsById = new Map<string, Extract<LongRunLedgerRecord, { type: "attempt_end" }>[]>();
  ...
  const attempts: LongRunAttemptProgress[] = plan.attempts.map((spec) => {
    ...
    for (const end of ends) {
      ...
      spentUsd += cost;
      if (end.costSource === "actual") measuredCostUsd += cost;
      else estimateFallbackCostUsd += cost;
      if (Number.isFinite(end.durationMs) && end.durationMs >= 0) durationsMs.push(end.durationMs);
    }
 ...
 */
export function redactLongRunText(text: string, env: NodeJS.ProcessEnv = process.env): string {
...
export function longRunBaseDir(root: string): string {
  return join(resolve(root), ...LONG_RUN_RELATIVE_DIR.split("/"));
}

function longRunPaths(root: string, runId: string): LongRunPaths {
  const runDir = join(longRunBaseDir(root), runId);
  return {
    runId,
    runDir,
    ...
    lockPath: join(runDir, "lock.json"),
    receiptsDir: join(runDir, "receipts"),
    costsDir: join(runDir, "costs"),
  };
}

function latestPointerPath(root: string): string {
  return join(longRunBaseDir(root), "latest");
}

/** Resolve --run <id> or the `latest` pointer. Undefined = no run found. */
export function resolveLongRunPaths(root: string, runId?: string): LongRunPaths | undefined {
  let id = runId;
  if (!id) {
  ...
  if (!existsSync(paths.runDir)) return undefined;
  return paths;
}
...
function writeLock(lockPath: string, startedAt: string): void {
  const lock: LongRunLock = {
    schema: "proofloop-longrun-lock-v1",
    pid: process.pid,
    host: hostname(),
    startedAt,
...
export type LongRunLockState =
  | { state: "none" }
  | { state: "live"; lock: LongRunLock }
  | { state: "stale"; lock: LongRunLock | undefined; reason: string };
 ...
 */
export function longRunLockState(lockPath: string, now: Date = new Date()): LongRunLockState {
  if (!existsSync(lockPath)) return { state: "none" };
  const lock = readLongRunLock(lockPath);
  if (!lock) return { state: "stale", lock: undefined, reason: "lock file is unparseable" };
  const heartbeatAge = now.getTime() - Date.parse(lock.heartbeatAt);
  if (!Number.isFinite(heartbeatAge) || heartbeatAge > LONG_RUN_STALE_LOCK_MS) {
    return { state: "stale", lock, reason: `heartbeat is ${Number.isFinite(heartbeatAge) ? `${Math.round(heartbeatAge / 1000)}s` : "unreadably"} old (> ${LONG_RUN_STALE_LOCK_MS / 60_000} min)` };
  }
  if (lock.host === hostname() && !pidAlive(lock.pid)) {
...
// ---------------------------------------------------------------------------
// state.json snapshot (convenience only; ledger.jsonl is authoritative)

type LongRunStateSnapshot = {
  schema: "proofloop-longrun-state-v1";
  runId: string;
  createdAt: string;
  status: "created" | "running" | "completed" | "failed" | "budget_exhausted" | "interrupted";
  updatedAt: string;
  authoritative: "ledger.jsonl";
  note: string;
};

function writeStateSnapshot(paths: LongRunPaths, status: LongRunStateSnapshot["status"], createdAt?: string): void {
  let created = createdAt;
  if (!created && existsSync(paths.statePath)) {
    try {
      const prior = JSON.parse(readFileSync(paths.statePath, "utf8"));
      if (isPlainObject(prior) && typeof prior.createdAt === "string") created = prior.createdAt;
    } catch {
      // fall through to now()
    }
  }
  const snapshot: LongRunStateSnapshot = {
    schema: "proofloop-longrun-state-v1",
    runId: paths.runId,
  ...
  writeFileSync(paths.statePath, `${JSON.stringify(snapshot, null, 2)}\n`, "utf8");
}
...
export function runLongRunInit(options: { root: string; planPath?: string; runId?: string } & LongRunIo): number {
  ...
  if (!options.planPath) {
    ...
    return LONG_RUN_EXIT.unusable;
  }
  const planFile = resolve(root, options.planPath);
  if (!existsSync(planFile)) {
    logError(`proofloop run init: plan file not found at ${planFile} (fail-closed).`);
    return LONG_RUN_EXIT.unusable;
  }

  let plan: LongRunPlan;
  try {
    plan = parseLongRunPlan(JSON.parse(readFileSync(planFile, "utf8")));
  } catch (error) {
  ...
  mkdirSync(paths.costsDir, { recursive: true });
  ...
  writeFileSync(latestPointerPath(root), `${runId}\n`, "utf8");
  ...
  if (totalEst > plan.budgetUsd) {
    log(`  WARNING: plan estimates (${usd(totalEst)}) exceed the budget (${usd(plan.budgetUsd)}). The run will stop honestly at the budget line (exit ${LONG_RUN_EXIT.budgetExhausted}, budget_exhausted in the ledger) -- it will NOT silently skip attempts.`);
  }
...
// ---------------------------------------------------------------------------
// executor -- `proofloop run start|resume`
...
function spawnAttemptTry(
  ...
  return new Promise((resolvePromise) => {
    ...
    let timedOut = false;
    let settled = false;
    const settle = (verdict: LongRunVerdict, exitCode: number | null) => {
      if (settled) return;
      settled = true;
    ...
    child.stdout?.on("data", (chunk: Buffer) => {
      stdoutTail = capTail(stdoutTail + chunk.toString("utf8"), LONG_RUN_STREAM_CAPTURE_MAX_CHARS);
    });
    child.stderr?.on("data", (chunk: Buffer) => {
      stderrTail = capTail(stderrTail + chunk.toString("utf8"), LONG_RUN_STREAM_CAPTURE_MAX_CHARS);
    });
    child.on("error", (error) => {
      stderrTail = capTail(`${stderrTail}\n[spawn error] ${error.message}`, LONG_RUN_STREAM_CAPTURE_MAX_CHARS);
      settle("error", null);
    });
    child.on("close", (code) => {
...
function readActualCost(costFile: string): { costUsd: number; costSource: "actual" } | undefined {
  if (!existsSync(costFile)) return undefined;
  try {
    const parsed = JSON.parse(readFileSync(costFile, "utf8"));
...
export function executeLongRun(
  options: { root: string; mode: "start" | "resume"; runId?: string; clearStaleLock?: boolean } & LongRunIo,
): Promise<number> {
  return executeLongRunInner(options);
}

async function executeLongRunInner(
  options: { root: string; mode: "start" | "resume"; runId?: string; clearStaleLock?: boolean } & LongRunIo,
): Promise<number> {
  const log = options.log ?? console.log;
  const logError = options.logError ?? console.error;
  ...
  let plan: LongRunPlan;
  try {
    plan = parseLongRunPlan(JSON.parse(readFileSync(paths.planPath, "utf8")));
  } catch (error) {
    logError(`${label}: stored plan is invalid: ${error instanceof Error ? error.message : String(error)} (fail-closed).`);
    return LONG_RUN_EXIT.unusable;
  }

  let ledger: LongRunLedgerReadResult;
  try {
  ...
  if (progress.remaining === 0) {
    ...
    return progress.failedTerminal === 0 ? LONG_RUN_EXIT.allPassed : LONG_RUN_EXIT.failures;
  }
  ...
  if (lockState.state === "live") {
    logError(
      `${label}: refusing -- a live runner holds the lock (pid ${lockState.lock.pid} on ${lockState.lock.host}, heartbeat ${lockState.lock.heartbeatAt}). Two runners would double-execute attempts and double-spend the budget.`,
    );
    return LONG_RUN_EXIT.unusable;
  }
  if (lockState.state === "stale") {
    if (!options.clearStaleLock) {
      logError(
        `${label}: a STALE lock is present (${lockState.reason}). If you are sure no runner is alive, re-run with --clear-stale-lock. Refusing to guess (fail-closed).`,
      );
      return LONG_RUN_EXIT.unusable;
    }
  ...
  if (ledger.droppedTornTail) {
    truncateSync(paths.ledgerPath, ledger.repairByteLength);
  ...
  }, LONG_RUN_HEARTBEAT_MS);
  heartbeat.unref();

  appendLongRunLedger(paths.ledgerPath, { type: "run_started", ts: startedAt, pid: process.pid, host: hostname(), mode: options.mode });
  ...
  const onSignal = (signal: NodeJS.Signals) => {
    ...
    process.exit(LONG_RUN_EXIT.interrupted);
  };
  ...
  const runOneTry = async (attempt: LongRunAttemptProgress): Promise<void> => {
    const spec = attempt.spec;
    const tryNumber = attempt.nextTry;
    attempt.nextTry += 1;
    const receiptFile = join(paths.receiptsDir, `${sanitizeFileStem(spec.id)}.try${tryNumber}.json`);
    const costFile = join(paths.costsDir, `${sanitizeFileStem(spec.id)}.try${tryNumber}.json`);
    ...
    const outcome = await spawnAttemptTry(
      spec,
      {
        ...process.env,
    ...
    if (interrupted) return; // the signal handler already recorded + exited path; be inert.

    const actual = readActualCost(costFile);
    const costUsd = actual ? actual.costUsd : spec.estCostUsd;
    const costSource: "actual" | "estimate" = actual ? "actual" : "estimate";
    const endRecord: Extract<LongRunLedgerRecord, { type: "attempt_end" }> = {
      type: "attempt_end",
      ts: new Date().toISOString(),
      attemptId: spec.id,
      try: tryNumber,
      ...
      costUsd,
      costSource,
      receiptPath: existsSync(receiptFile) ? relativeToRunDir(paths, receiptFile) : null,
      stdoutTail: capTail(redactLongRunText(outcome.stdoutTail), LONG_RUN_SNIPPET_MAX_CHARS),
      stderrTail: capTail(redactLongRunText(outcome.stderrTail), LONG_RUN_SNIPPET_MAX_CHARS),
    };
    ...
    spentUsd += costUsd;
    log(
      `${label}: [${spec.id}] try ${tryNumber}/${plan.maxRetries + 1} ${outcome.verdict.toUpperCase()} (exit ${outcome.exitCode ?? "spawn-error"}, ${outcome.durationMs}ms, ${usd(costUsd)} ${costSource}) -- spent ${usd(spentUsd)}/${usd(plan.budgetUsd)}`,
    );
  };
      ...
      while (!attempt.terminal && !interrupted) {
        ...
        if (spentUsd + reservedUsd + attempt.spec.estCostUsd > plan.budgetUsd + 1e-9) {
          if (!budgetStopped) {
            ...
            logError(
              `${label}: BUDGET EXHAUSTED -- spent ${usd(spentUsd)} of ${usd(plan.budgetUsd)}; next try of [${attempt.spec.id}] estimates ${usd(attempt.spec.estCostUsd)} and would cross the line. Stopping honestly (exit ${LONG_RUN_EXIT.budgetExhausted}); NOT silently skipping. Raise budgetUsd in ${relativeToRunDir(paths, paths.planPath)} and \`proofloop run resume\` to continue.`,
            );
          }
          return;
        }
  ...
  if (budgetStopped) {
    ...
    return LONG_RUN_EXIT.budgetExhausted;
  }
  appendLongRunLedger(paths.ledgerPath, {
    type: "run_completed",
    ts: new Date().toISOString(),
  ...
  printProgressSummary(log, plan, final);
  return final.failedTerminal === 0 ? LONG_RUN_EXIT.allPassed : LONG_RUN_EXIT.failures;
}

function sanitizeFileStem(id: string): string {
  return id.replace(/[^A-Za-z0-9._-]/g, "_");
}

function relativeToRunDir(paths: LongRunPaths, absolute: string): string {
  return absolute.startsWith(paths.runDir) ? absolute.slice(paths.runDir.length + 1).replace(/\\/g, "/") : absolute;
}

function printProgressSummary(log: (line: string) => void, plan: LongRunPlan, progress: LongRunProgress): void {
...
export function runLongRunStatus(options: { root: string; runId?: string; clearStaleLock?: boolean } & LongRunIo): number {
  const log = options.log ?? console.log;
  const logError = options.logError ?? console.error;
  const root = resolve(options.root);
  ...
  if (lockState.state === "live") {
    ...
    if (inFlight.length > 0) log(`  current:  ${inFlight.join(", ")} (attempt_start without attempt_end while a live runner holds the lock)`);
    ...
    if (unfinished.length > 0) log(`  note:     unfinished tries from a dead runner: ${unfinished.join(", ")} -- they re-run on resume.`);
  }
  return 0;
}
...
export function buildLongRunReport(runId: string, plan: LongRunPlan, records: LongRunLedgerRecord[], now: Date = new Date()): LongRunReport {
  ...
  const emptyBucket = (): Bucket => ({ attempts: 0, passed: 0, failed: 0, remaining: 0, spentUsd: 0, measuredCostUsd: 0, durations: [] });
  ...
  for (const attempt of progress.attempts) {
    for (const [key, map] of [
      ...
      else if (attempt.terminal) bucket.failed += 1;
      else bucket.remaining += 1;
      for (const end of attempt.ends) {
        bucket.spentUsd += end.costUsd;
        if (end.costSource === "actual") bucket.measuredCostUsd += end.costUsd;
  ...
  const rate = (bucket: Bucket): number | null => {
    const terminal = bucket.passed + bucket.failed;
    return terminal === 0 ? null : round4(bucket.passed / terminal);
  };
  const perPass = (bucket: Bucket): number | null => (bucket.passed === 0 ? null : round4(bucket.spentUsd / bucket.passed));

  const families: LongRunFamilyReportRow[] = [...familyBuckets.entries()].map(([id, bucket]) => ({
    id,
  ...
  const modelSummaries: LongRunModelReportRow[] = [...modelBuckets.entries()].map(([modelId, bucket]) => ({
    modelId,
    attempts: bucket.attempts,
    passed: bucket.passed,
    failed: bucket.failed,
  ...
  return {
    schema: "proofloop-longrun-report-v1",
    framing: LONG_RUN_REPORT_FRAMING,
    runId,
...
export function formatLongRunReportMarkdown(report: LongRunReport): string {
  const pct = (value: number | null): string => (value === null ? "n/a" : `${(value * 100).toFixed(1)}%`);
  const money = (value: number | null): string => (value === null ? "n/a" : usd(value));
  const lines: string[] = [
    ...
    "|---|---|---|---|---|---|---|---|",
    ...report.families.map(
      (row) => `| ${row.id} | ${row.attempts} | ${row.passed} | ${row.failed} | ${row.remaining} | ${pct(row.passRate)} | ${usd(row.spentUsd)} | ${money(row.costPerPassUsd)} |`,
    ),
    "",
    "## Per model",
    "",
    ...
    "|---|---|---|---|---|---|---|---|---|---|",
    ...report.modelSummaries.map(
      (row) =>
        `| ${row.modelId} | ${row.attempts} | ${row.passed} | ${row.failed} | ${row.remaining} | ${pct(row.passRate)} | ${usd(row.spentUsd)} | ${usd(row.measuredCostUsd)} | ${row.avgDurationMs ?? "n/a"} | ${money(row.costPerPassUsd)} |`,
    ),
    "",
  ];
  return lines.join("\n");
...
export function runLongRunReport(options: { root: string; runId?: string; json?: boolean } & LongRunIo): number {
  const log = options.log ?? console.log;
  const logError = options.logError ?? console.error;
  const root = resolve(options.root);
  ...
  writeFileSync(join(paths.runDir, "report.md"), markdown, "utf8");
