...
export const HOSTED_SCHEMA_VERSION = "proofloop-hosted-run-v1";

export type HostedAppType =
  | "agent-app"
  | "chat-agent"
  | "workflow-agent"
  | "spreadsheet-agent"
...
export function validateHostedRunRequest(
  request: HostedRunRequest,
  options: { allowlistedHosts?: string[] } = {},
): { ok: boolean; blockers: string[]; warnings: string[] } {
  const blockers: string[] = [];
  const warnings: string[] = [];
  const url = new URL(request.targetUrl);
  if (url.protocol !== "https:" && !isLocalhost(url.hostname)) blockers.push("target_url_must_be_https_for_hosted_runs");
  if (isPrivateNetworkHost(url.hostname) && !isLocalhost(url.hostname)) blockers.push("private_network_targets_are_not_allowed_from_hosted_runs");
  if (!request.consent.accepted) blockers.push("consent_checkbox_required");
  if (!request.consent.ownsOrAuthorized) blockers.push("ownership_or_authorization_required");
  if (!request.consent.allowBrowserAutomation) blockers.push("browser_automation_consent_required");
  if (!request.consent.allowRecording) blockers.push("artifact_recording_consent_required");
  if (!finiteNonNegative(request.modelBudgetUsd)) blockers.push("model_budget_must_be_non_negative");
  if (request.modelBudgetUsd > 100) warnings.push("budget_above_default_guardrail_requires_manual_review");
  if (containsSecretLikeText(request.authNotes)) warnings.push("auth_notes_may_contain_secret_material_do_not_store_raw_passwords");

  const permission = verifyHostedDomainPermission(request, options);
  if (permission.status !== "verified") blockers.push(...permission.blockers);
  return { ok: blockers.length === 0, blockers, warnings };
}

export function verifyHostedDomainPermission(
  request: HostedRunRequest,
  options: { allowlistedHosts?: string[] } = {},
...
export function buildHostedSuccessContract(request: HostedRunRequest): HostedSuccessContract {
  const host = new URL(request.targetUrl).hostname.toLowerCase();
...
export function composeHostedBenchmarkProxyTasks(request: HostedRunRequest): HostedBenchmarkProxyTask[] {
...
export function buildHostedRunBundle(options: HostedIntakeOptions): HostedRunBundle {
...
export function buildHostedRunBundleFromRequest(
  request: HostedRunRequest,
  options: { allowlistedHosts?: string[] } = {},
): HostedRunBundle {
  ...
  const targetPlan = buildProofloopTargetPlan({
    root: process.cwd(),
    urlSignals: {
  ...
  const bundle: Omit<HostedRunBundle, "dashboardHtml"> = {
    schema: "proofloop-hosted-run-bundle-v1",
    runId,
    generatedAt: request.createdAt,
    request,
...
export function readHostedBundleReference(path: string, options: { root?: string } = {}): HostedRunBundle {
  ...
  if (parsed.schema === "proofloop-hosted-run-bundle-v1") return readHostedRunBundle(absolutePath);
  if (parsed.bundlePath) return readHostedRunBundle(resolve(root, parsed.bundlePath));
  if (parsed.requestPath) {
    const request = JSON.parse(readFileSync(resolve(root, parsed.requestPath), "utf8")) as HostedRunRequest;
    return buildHostedRunBundleFromRequest(request);
  }
  if (parsed.schema === HOSTED_SCHEMA_VERSION) return buildHostedRunBundleFromRequest(parsed as unknown as HostedRunRequest);
  throw new Error(`Unsupported hosted run reference: ${path}`);
}

export function buildHostedWorkerPlan(
...
export function writeHostedWorkerPlan(options: {
  requestFile: string;
  root?: string;
  outFile?: string;
  generatedAt?: string;
}): { bundle: HostedRunBundle; plan: HostedWorkerPlan; file: string } {
  const root = resolve(options.root ?? process.cwd());
  ...
  const file = writeJson(resolve(root, options.outFile ?? join(bundle.runner.artifactRoot, "worker-plan.json")), plan);
  return { bundle, plan, file };
}

export function writeHostedRunBundle(options: WriteHostedRunBundleOptions): { bundle: HostedRunBundle; files: string[] } {
  const root = resolve(options.root ?? process.cwd());
  const bundle = buildHostedRunBundle(options);
  const outDir = resolve(root, options.outDir ?? join(".proofloop", "hosted", "requests", bundle.runId));
  const files = [
    writeJson(join(outDir, "request.json"), bundle.request),
    writeJson(join(outDir, "domain-permission.json"), bundle.permission),
    writeJson(join(outDir, "success-contract.json"), bundle.contract),
    writeJson(join(outDir, "run-bundle.json"), { ...bundle, dashboardHtml: undefined }),
    writeText(join(outDir, "dashboard.html"), bundle.dashboardHtml),
    writeText(join(outDir, "runbook.md"), renderHostedRunbook(bundle)),
  ];
  const queuePath = resolve(root, bundle.runner.queuePath);
  files.push(writeJson(queuePath, { runId: bundle.runId, requestPath: relativeLike(root, join(outDir, "request.json")), bundlePath: relativeLike(root, join(outDir, "run-bundle.json")) }));
  return { bundle, files };
}

export function renderHostedRunbook(bundle: HostedRunBundle): string {
  return [
...
export function renderHostedDashboardHtml(bundle: Omit<HostedRunBundle, "dashboardHtml">): string {
...
function contractPreset(appType: HostedAppType): Pick<HostedSuccessContract["success"], "visibleTextAny" | "visibleTestIdAny" | "urlIncludesAny"> {
  if (appType === "chat-agent") return { visibleTextAny: ["response", "message", "assistant"], visibleTestIdAny: ["chat-composer", "message-composer"], urlIncludesAny: [] };
  if (appType === "workflow-agent") return { visibleTextAny: ["complete", "done", "task", "workflow"], visibleTestIdAny: ["run-status", "task-result"], urlIncludesAny: [] };
  if (appType === "spreadsheet-agent") return { visibleTextAny: ["sheet", "cell", "formula", "workbook"], visibleTestIdAny: ["sheet-grid", "workbook"], urlIncludesAny: [] };
  if (appType === "research-copilot") return { visibleTextAny: ["source", "citation", "research", "memo"], visibleTestIdAny: ["citation", "source-panel"], urlIncludesAny: [] };
  if (appType === "underwriting-agent") return { visibleTextAny: ["risk", "memo", "decision", "underwriting"], visibleTestIdAny: ["decision-memo", "risk-panel"], urlIncludesAny: [] };
  if (appType === "accounting-agent") return { visibleTextAny: ["ledger", "trial balance", "reconciliation", "journal"], visibleTestIdAny: ["ledger", "sheet-grid"], urlIncludesAny: [] };
  if (appType === "document-memory") return { visibleTextAny: ["document", "memory", "chunk", "source"], visibleTestIdAny: ["upload", "source-panel"], urlIncludesAny: [] };
  return { visibleTextAny: ["complete", "done", "result"], visibleTestIdAny: ["chat-composer", "run-status", "result"], urlIncludesAny: [] };
}

function familiesForAppType(appType: HostedAppType): string[] {
  if (appType === "accounting-agent") return ["bankertoolbench", "spreadsheetbench-v1"];
  if (appType === "spreadsheet-agent") return ["spreadsheetbench-v1", "spreadsheetbench-v2"];
  if (appType === "research-copilot") return ["research-copilot", "finch"];
  if (appType === "underwriting-agent") return ["proximitty-underwriting"];
  if (appType === "workflow-agent") return ["workstreambench"];
  if (appType === "document-memory") return ["nodeagent-memory-ingestion"];
  if (appType === "chat-agent") return ["live-browser-smoke", "workstreambench"];
  return ["live-browser-smoke"];
}

function tasksForFamily(family: string, request: HostedRunRequest): HostedBenchmarkProxyTask[] {
  const audience = request.intendedAudience;
  const boundary: HostedBenchmarkProxyTask["officialBoundary"] = family === "live-browser-smoke" || family === "research-copilot" || family === "proximitty-underwriting" || family === "nodeagent-memory-ingestion"
    ? "proxy_product_path"
    : "official_scorer_required";
  const make = (id: string, title: string, prompt: string, successSignal: string): HostedBenchmarkProxyTask => ({
    id,
    family,
    title,
    audience,
...
function normalizeHttpUrl(raw: string): string {
  ...
  if (url.protocol !== "http:" && url.protocol !== "https:") throw new Error("target URL must start with http:// or https://");
  return url.toString();
}
...
function hostedDomainToken(host: string): string {
  return `proofloop-domain-${safeId(host)}-verify`;
}

function hostedRunId(request: HostedRunRequest): string {
...
function relativeLike(root: string, path: string): string {
...
export function readHostedRunBundle(path: string): HostedRunBundle {
  const parsed = JSON.parse(readFileSync(path, "utf8")) as HostedRunBundle;
