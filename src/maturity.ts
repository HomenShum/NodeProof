...
type RepoSignals = {
  ...
  text: string;
  config: ProofloopConfigSnapshot;
};

type ProofloopConfigSnapshot = {
  exists: boolean;
  gateChecks: number;
  immutable: number;
...
const LEVELS: Omit<AgentEraLevelAssessment, "status">[] = [
  {
    level: 0,
...
const DEFAULT_REPORT_PATH = join(".proofloop", "reports", "agent-era-maturity.md");
const DEFAULT_JSON_PATH = join(".proofloop", "reports", "agent-era-maturity.json");
const MAX_FILES = 900;
const MAX_TEXT_BYTES = 220_000;

export function assessAgentEraMaturity(options: AgentEraMaturityOptions): AgentEraMaturityReport {
  ...
  const report: Omit<AgentEraMaturityReport, "reportMarkdown"> = {
    schema: "proofloop-agent-era-maturity-v1",
    generatedAt,
    root,
...
export function writeAgentEraMaturityReport(
  options: AgentEraMaturityOptions & { outPath?: string },
): WriteAgentEraMaturityResult {
  const root = resolve(options.root);
  const report = assessAgentEraMaturity(options);
  const markdownPath = resolve(root, options.outPath ?? DEFAULT_REPORT_PATH);
  const jsonPath = resolve(root, DEFAULT_JSON_PATH);
  mkdirSync(dirname(markdownPath), { recursive: true });
  mkdirSync(dirname(jsonPath), { recursive: true });
  writeFileSync(markdownPath, report.reportMarkdown, "utf8");
  writeFileSync(jsonPath, `${JSON.stringify(withoutMarkdown(report), null, 2)}\n`, "utf8");
  return { report, markdownPath, jsonPath };
}

export function formatAgentEraMaturityDense(report: AgentEraMaturityReport): string {
...
function buildCapabilities(signals: RepoSignals): AgentEraCapability[] {
...
function capability(args: AgentEraCapability): AgentEraCapability {
...
function buildLevelAssessments(capabilities: AgentEraCapability[]): AgentEraLevelAssessment[] {
  const byId = new Map(capabilities.map((capability) => [capability.id, capability]));
  return LEVELS.map((level) => {
    const required = level.requiredCapabilityIds.map((id) => byId.get(id)).filter((entry): entry is AgentEraCapability => entry !== undefined);
    const status: AgentEraCapabilityStatus = required.length === 0
      ? "met"
...
function currentCompletedLevel(levels: AgentEraLevelAssessment[]): AgentEraLevelId {
  let current: AgentEraLevelId = 0;
  for (const level of levels.filter((entry) => entry.level > 0).sort((a, b) => a.level - b.level)) {
    if (level.status !== "met") break;
    current = level.level;
  }
  return current;
}

function maturityScore(capabilities: AgentEraCapability[]): number {
...
function buildNextActions(capabilities: AgentEraCapability[], currentLevel: AgentEraLevelId, targetLevel: AgentEraLevelId): string[] {
...
function renderMaturityMarkdown(report: Omit<AgentEraMaturityReport, "reportMarkdown">): string {
...
function renderTimelineMermaid(): string {
  return [
    "flowchart LR",
...
function renderProjectionMermaid(currentLevel: AgentEraLevelId, targetLevel: AgentEraLevelId): string {
  const lines = [
    "xychart-beta",
...
function readRepoSignals(root: string): RepoSignals {
  ...
  return {
    root,
    repoName: pkg.name?.trim() || basename(root),
    files,
    scripts,
    dependencyNames,
    text,
...
function readPackageJson(root: string): PackageJson {
  ...
  try {
    const parsed = JSON.parse(readFileSync(path, "utf8").replace(/^\uFEFF/, "")) as PackageJson;
...
function readProofloopConfig(root: string): ProofloopConfigSnapshot {
  const path = join(root, "proofloop.config.json");
  if (!existsSync(path)) return { exists: false, gateChecks: 0, immutable: 0, protectedPaths: 0 };
  try {
...
function collectSearchText(root: string, files: string[], pkg: PackageJson): string {
...
function shouldReadForSignals(file: string): boolean {
  if (/^(README|AGENTS|CLAUDE)\.md$/i.test(file)) return true;
  if (/^docs\/agent-os\/[^/]+\.md$/i.test(file)) return true;
  if (/^api\/hosted\/[^/]+\.js$/i.test(file)) return true;
  if (/^src\/[^/]+\.(ts|tsx|js|jsx|mjs)$/i.test(file)) return true;
  if (/^scripts\/[^/]+\.(mjs|js|ts)$/i.test(file)) return true;
  if (/^public\/index\.html$/i.test(file)) return true;
  if (/^tests\/[^/]+\.(ts|tsx|js)$/i.test(file)) return true;
  if (/^\.github\/workflows\/[^/]+\.ya?ml$/i.test(file)) return true;
  if (/proofloop|benchmark|scorer|playwright|cypress|memory|trace|receipt/i.test(file)) return true;
  return false;
}

function hasScript(signals: RepoSignals, pattern: RegExp): boolean {
  return Object.entries(signals.scripts).some(([name, command]) => pattern.test(`${name} ${command}`));
}

function matchingScripts(signals: RepoSignals, pattern: RegExp): string[] {
  return Object.entries(signals.scripts)
    .filter(([name, command]) => pattern.test(`${name} ${command}`))
    .map(([name, command]) => `script: ${name} -> ${command}`);
}

function hasFile(signals: RepoSignals, pattern: RegExp): boolean {
  return signals.files.some((file) => pattern.test(file));
}

function matchingFiles(signals: RepoSignals, pattern: RegExp): string[] {
  return signals.files.filter((file) => pattern.test(file)).map((file) => `file: ${file}`);
}

function hasAgentDocs(signals: RepoSignals): boolean {
  return hasFile(signals, /^(AGENTS\.md|CLAUDE\.md|\.cursor\/rules\/|\.windsurf\/rules\/|docs\/agent-os\/README\.md)/);
}

function hasUiContracts(signals: RepoSignals): boolean {
  return textIncludes(signals.text, /data-testid|data-proofloop|expected-tool-use/i);
}

function hasOfficialScorer(signals: RepoSignals): boolean {
...
function hasBenchmarkEvidence(signals: RepoSignals): boolean {
...
function hasMemoryCommand(text: string): boolean {
...
function matchingTextEvidence(text: string, patterns: RegExp[]): string[] {
  const evidence: string[] = [];
  for (const pattern of patterns) {
    if (pattern.test(text)) evidence.push(`text: ${pattern.source.replace(/\\/g, "")}`);
  }
  return evidence;
}
...
function normalizeTargetLevel(value: number | undefined): AgentEraLevelId {
  if (value === undefined || !Number.isFinite(value)) return 5;
  return Math.max(0, Math.min(5, Math.trunc(value))) as AgentEraLevelId;
}

function statusRank(status: AgentEraCapabilityStatus): number {
  return status === "met" ? 2 : status === "partial" ? 1 : 0;
}

function withoutMarkdown(report: AgentEraMaturityReport): Omit<AgentEraMaturityReport, "reportMarkdown"> {
