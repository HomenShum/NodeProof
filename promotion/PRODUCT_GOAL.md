...
| 10 | Performance does not obstruct interaction | PASS | `domContentLoaded` 206 ms, `load` 206 ms, 9 874 B same-origin transfer on first paint; J5's click-to-status latency 211 ms including the `.well-known` and DNS verification round trips ([evidence/browser-proof/receipt.json](evidence/browser-proof/receipt.json) → `timings`, `journeys.J5.latencyMs`). Read from `performance.getEntriesByType("navigation")` in the page, not estimated from file sizes, and the probe exits 1 above a 3 000 ms `domContentLoaded` budget. |
