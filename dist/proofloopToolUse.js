...
exports.TOOL_USE_CONTRACT_TEMPLATES = exports.ToolUseTraceError = exports.ToolUseContractError = exports.DEFAULT_MAX_MALFORMED_RATIO = exports.TOOL_USE_CONTRACT_VERSION = exports.TOOL_USE_LOG_RELATIVE_PATH = void 0;
exports.splitMcpToolName = splitMcpToolName;
exports.parseToolUseContract = parseToolUseContract;
exports.loadToolUseContract = loadToolUseContract;
exports.parseToolUseTrace = parseToolUseTrace;
exports.loadToolUseTrace = loadToolUseTrace;
exports.matchParamsSubset = matchParamsSubset;
exports.verifyToolUseContract = verifyToolUseContract;
exports.formatToolUseVerdict = formatToolUseVerdict;
exports.runToolUseVerify = runToolUseVerify;
exports.runToolUseInit = runToolUseInit;
/**
 * Proof Loop expected-tool-use contracts -- `proofloop tooluse <verify|init>`.
 *
 * PORTED VERBATIM from the noderoom reference implementation
 * (src/eval/proofloopToolUse.ts). The security-critical properties below are
 * load-bearing and MUST NOT be regressed:
 *   - empty_trace fail-closed: a forbidden/order contract against a zero-call
 *     trace FAILS (never vacuously passes).
 *   - server-pinned namespace matching: mcp__evil__ cannot satisfy
 *     server: "composio".
 *   - block-biased forbidden matching (full + bare name).
 *   - implicit regex anchoring (^(?:...)$).
 *   - prototype-pollution-inert own-key reads.
 *   - fail-closed loaders (missing/unparseable => throw, CLI exit 2).
 *
 ...
 * written by `proofloop hooks install` (see proofloopHooks.ts) captures every
 * tool call into `.proofloop/tooluse/log.jsonl`, and this module's
 * DETERMINISTIC verifier checks that capture against a declared contract:
 *
 ...
 * verifier that failed open here would reward log sabotage. No LLM anywhere.
 *
 * Contracts are JSON (this package deliberately has no YAML parser dependency;
 * zero new npm deps).
 */
const node_fs_1 = require("node:fs");
const node_path_1 = require("node:path");
/** Repo-relative path the PostToolUse logger appends to (forward slashes). */
exports.TOOL_USE_LOG_RELATIVE_PATH = ".proofloop/tooluse/log.jsonl";
exports.TOOL_USE_CONTRACT_VERSION = 1;
exports.DEFAULT_MAX_MALFORMED_RATIO = 0.1;
/** Contract file is unusable (missing, unparseable, invalid). CLI exit 2. */
class ToolUseContractError extends Error {
}
exports.ToolUseContractError = ToolUseContractError;
/** Trace file is unusable (missing / unreadable). CLI exit 2. */
class ToolUseTraceError extends Error {
}
exports.ToolUseTraceError = ToolUseTraceError;
// ---------------------------------------------------------------------------
...
function splitMcpToolName(name) {
    ...
    if (match)
        return { server: match[1], bareTool: match[2] };
    return { server: null, bareTool: name };
}
function requiredNameMatches(rule, call) {
    if (rule.server !== undefined && call.server !== rule.server)
        return false;
    return rule.tool.matches(call.bareTool) || rule.tool.matches(call.tool);
}
function forbiddenMatches(rule, call) {
    if (rule.server !== undefined && call.server !== rule.server)
        return false;
    return rule.tool.matches(call.tool) || rule.tool.matches(call.bareTool);
}
function orderRefMatches(ref, call) {
...
function compileAnchored(source, context) {
    try {
...
function compileToolRef(value, context) {
...
function isPatternLeaf(value) {
    if (!isPlainObject(value))
        return false;
    const keys = Object.keys(value);
    return keys.length === 1 && keys[0] === "pattern" && typeof ownGet(value, "pattern") === "string";
}
function validateParamsMatcher(node, context) {
    ...
    if (isPlainObject(node)) {
        for (const key of Object.keys(node))
            validateParamsMatcher(ownGet(node, key), `${context}.${key}`);
        return;
    }
...
function rejectUnknownKeys(record, allowed, context) {
    for (const key of Object.keys(record)) {
        if (!allowed.includes(key)) {
            throw new ToolUseContractError(`${context}: unknown key "${key}" (allowed: ${allowed.join(", ")}). Refusing to guess -- a typo here would silently weaken the contract.`);
        }
    }
}
function parseServerPin(value, context) {
    if (value === undefined)
        return undefined;
    if (typeof value !== "string" || value.length === 0) {
...
function parseToolUseContract(value) {
    if (!isPlainObject(value))
        throw new ToolUseContractError("contract must be a JSON object.");
    rejectUnknownKeys(value, ["version", "required", "forbidden", "order", "session", "maxMalformedRatio", "$comment", "description"], "contract");
    if (ownGet(value, "version") !== exports.TOOL_USE_CONTRACT_VERSION) {
        throw new ToolUseContractError(`contract.version must be exactly ${exports.TOOL_USE_CONTRACT_VERSION} (got ${JSON.stringify(ownGet(value, "version"))}).`);
    }
    const required = asRuleArray(ownGet(value, "required"), "contract.required").map((entry, i) => {
        ...
        if (paramsRaw !== undefined)
            validateParamsMatcher(paramsRaw, `${context}.params`);
        const note = ownGet(entry, "note");
        if (note !== undefined && typeof note !== "string")
            throw new ToolUseContractError(`${context}.note must be a string.`);
        return {
            ...
            ...(paramsRaw !== undefined ? { params: paramsRaw } : {}),
            ...(note !== undefined ? { note } : {}),
        };
    });
    const forbidden = asRuleArray(ownGet(value, "forbidden"), "contract.forbidden").map((entry, i) => {
    ...
    const order = asRuleArray(ownGet(value, "order"), "contract.order").map((entry, i) => {
    ...
    let maxMalformedRatio = exports.DEFAULT_MAX_MALFORMED_RATIO;
    ...
    return {
        version: exports.TOOL_USE_CONTRACT_VERSION,
        required,
        forbidden,
        order,
...
function asRuleArray(value, context) {
    if (value === undefined)
        return [];
    if (!Array.isArray(value))
        throw new ToolUseContractError(`${context} must be an array.`);
    return value.map((entry, i) => {
        if (!isPlainObject(entry))
            throw new ToolUseContractError(`${context}[${i}] must be an object.`);
        return entry;
    });
}
...
function loadToolUseContract(path) {
    ...
    try {
        ...
        const yamlHint = /\.ya?ml$/i.test(path)
            ? " Contracts are JSON (this package has no YAML parser dependency)."
            : "";
...
// ---------------------------------------------------------------------------
// trace parsing
function parseToolUseTrace(text) {
    const calls = [];
    let totalLines = 0;
...
function loadToolUseTrace(path) {
    if (!(0, node_fs_1.existsSync)(path)) {
        throw new ToolUseTraceError(`trace file not found: ${path}. Run \`proofloop hooks install\` so the PostToolUse logger captures tool calls, or pass --trace <file>.`);
    }
    return parseToolUseTrace((0, node_fs_1.readFileSync)(path, "utf8"));
}
// ---------------------------------------------------------------------------
 ...
 */
function matchParamsSubset(matcher, actual, path = "params") {
    ...
    if (Array.isArray(matcher)) {
        if (!Array.isArray(actual))
            return `${path}: expected an array, got ${describeValue(actual)}`;
        if (actual.length !== matcher.length)
            return `${path}: expected array length ${matcher.length}, got ${actual.length}`;
...
function verifyToolUseContract(contract, trace, options = {}) {
...
function formatToolUseVerdict(verdict, context) {
    const lines = [
...
exports.TOOL_USE_CONTRACT_TEMPLATES = {
    "composio-email-triage": () => ({
        $comment: "Expected-tool-use contract for a Composio-MCP email-triage agent. Verified against LOCAL PostToolUse capture (.proofloop/tooluse/log.jsonl) -- not Composio server-side attestation.",
        version: 1,
        required: [
            {
...
function runToolUseVerify(options) {
    const log = options.log ?? console.log;
    const logError = options.logError ?? console.error;
    const contractPath = (0, node_path_1.resolve)(options.root, options.contractPath);
    const tracePath = (0, node_path_1.resolve)(options.root, options.tracePath ?? exports.TOOL_USE_LOG_RELATIVE_PATH);
    let contract;
    let trace;
    try {
    ...
    if (options.json) {
        ...
        log(formatToolUseVerdict(verdict, {
            contractPath,
            tracePath,
...
function runToolUseInit(options) {
    const log = options.log ?? console.log;
    const logError = options.logError ?? console.error;
    const templateName = options.template ?? "composio-email-triage";
    const factory = exports.TOOL_USE_CONTRACT_TEMPLATES[templateName];
    if (!factory) {
        logError(`proofloop tooluse: unknown template "${templateName}". Known: ${Object.keys(exports.TOOL_USE_CONTRACT_TEMPLATES).join(", ")}`);
        return 1;
    }
    ...
    const outPath = (0, node_path_1.resolve)(options.root, options.outPath ?? "tooluse-contract.json");
    (0, node_fs_1.mkdirSync)((0, node_path_1.dirname)(outPath), { recursive: true });
    (0, node_fs_1.writeFileSync)(outPath, `${JSON.stringify(template, null, 2)}\n`, "utf8");
