...
exports.TRANSFER_DIRECTION_LABELS = exports.TRANSFER_SAMPLE_SCHEMA = void 0;
exports.readTransferLaneResults = readTransferLaneResults;
exports.buildTransferSample = buildTransferSample;
exports.evaluateTransferGate = evaluateTransferGate;
exports.transferClaimLine = transferClaimLine;
exports.runTransferCheckCommand = runTransferCheckCommand;
/**
...
exports.TRANSFER_SAMPLE_SCHEMA = "proofloop-transfer-sample-v1";
exports.TRANSFER_DIRECTION_LABELS = {
...
const RECEIPT_KNOWN_KEYS = ["taskId", "model", "family", "pass"];
...
// ---------------------------------------------------------------------------
function readTransferLaneResults(path, options = {}) {
    if (!(0, node_fs_1.existsSync)(path))
        throw new Error(`lane input not found: ${path}`);
    const raw = (0, node_fs_1.readFileSync)(path, "utf8").replace(/^\uFEFF/, "");
    if (raw.trimStart().startsWith("["))
        return readReceiptsArray(raw, path);
    return readRunnerLedger(raw, path, options.ledgerModel);
}
function readReceiptsArray(raw, path) {
    let parsed;
    try {
    ...
    if (!Array.isArray(parsed))
        throw new Error(`receipts file ${path} must be a JSON array of { taskId, model, family?, pass }`);
    if (parsed.length === 0)
        throw new Error(`receipts file ${path} is an empty array -- zero evidence is unusable (fail-closed)`);
    const warnings = [];
    const seen = new Set();
    const results = parsed.map((entry, index) => {
        ...
        if (typeof taskId !== "string" || !taskId.trim())
            throw new Error(`receipts entry ${index} in ${path}: "taskId" (non-empty string) is required`);
        const model = record.model;
        if (typeof model !== "string" || !model.trim())
            throw new Error(`receipts entry ${index} in ${path}: "model" (non-empty string) is required`);
        const pass = record.pass;
        if (typeof pass !== "boolean")
            throw new Error(`receipts entry ${index} in ${path}: "pass" (boolean) is required -- refusing to coerce ${JSON.stringify(pass)}`);
        let family = "default";
...
function readRunnerLedger(raw, path, ledgerModel) {
    const warnings = [];
    ...
    for (let index = 0; index < lines.length; index++) {
        ...
        try {
            ...
            throw new Error(`${path}:${index + 1}: not a receipts JSON array and not a parseable runner-ledger line -- accepted inputs are a receipts array [{taskId,model,family?,pass}] or a ${RUNNER_EVENT_SCHEMA} events ledger (fail-closed)`);
        }
        ...
        const status = data && typeof data === "object" && !Array.isArray(data) ? data.status : undefined;
        if (status !== "passed" && status !== "failed") {
            throw new Error(`${path}:${index + 1}: task_completed data.status must be "passed" or "failed", got ${JSON.stringify(status)} (fail-closed; runner schema drift?)`);
        }
...
function byFamilyTaskModel(a, b) {
 ...
 */
function buildTransferSample(capability, options) {
    ...
    if (!Number.isInteger(perFamily) || perFamily < 1)
        throw new Error("--per-family must be an integer >= 1");
    if (capability.length === 0)
        throw new Error("capability set is empty -- nothing to sample (fail-closed)");
    ...
    for (const family of [...byFamily.keys()].sort()) {
        ...
        for (const failure of shuffled(failures, rand).slice(0, failureSlots))
            picked.add(failure);
        const rest = members.filter((member) => !picked.has(member));
        for (const member of shuffled(rest, rand).slice(0, slots - picked.size))
            picked.add(member);
    ...
    return { schema: exports.TRANSFER_SAMPLE_SCHEMA, seed, pairs };
}
// ---------------------------------------------------------------------------
// agreement gate
// ---------------------------------------------------------------------------
function evaluateTransferGate(capability, browser, options = {}) {
    ...
    if (!Number.isInteger(requestedMinOverlap))
        throw new Error("--min-overlap must be an integer");
    ...
    for (const result of capability)
        capabilityByKey.set(joinKey(result.taskId, result.model), result);
    const paired = [];
    for (const browserResult of [...browser].sort(byFamilyTaskModel)) {
        ...
        if (cap)
            paired.push({ cap, browser: browserResult });
    }
    ...
    const disagreements = paired
        ...
        .map((pair) => {
        const direction = pair.cap.pass ? "capability-pass-browser-fail" : "capability-fail-browser-pass";
        return {
            ...
            direction,
            label: exports.TRANSFER_DIRECTION_LABELS[direction],
        };
    });
    const base = {
        overlap,
    ...
    if (capabilityFailuresTotal > 0 && pairedCapabilityFailures === 0) {
        if (!options.allowNoFailureOverlap) {
            return {
                status: "unusable",
                reason: `cherry-pick guard: the capability set contains ${capabilityFailuresTotal} failure(s) but the browser paired set contains ZERO of them; ` +
        ...
        base.warnings.push(`WARNING: --allow-no-failure-overlap: the capability set contains ${capabilityFailuresTotal} failure(s) but the browser paired set includes NONE of them. ` +
 ...
 */
function transferClaimLine(evaluation) {
    return (`Capability verified through the live agent harness; production browser path verified by stratified UI certification ` +
...
function floatOption(value, flag) {
    const raw = strOption(value);
    if (raw === undefined) {
        if (value === true)
            throw new Error(`${flag} requires a value`);
...
function resolveLane(root, path) {
    return (0, node_path_1.isAbsolute)(path) ? path : (0, node_path_1.resolve)(root, path);
}
/**
 ...
 */
function runTransferCheckCommand(sub, options, root, io = {}) {
    const log = io.log ?? console.log;
    const logError = io.logError ?? console.error;
    try {
        if (sub === "sample")
            return runSample(options, root, log, logError);
        if (sub === "gate")
            return runGateSub(options, root, log, logError);
...
function runSample(options, root, log, logError) {
    const capabilityPath = strOption(options.capability);
    if (!capabilityPath)
        throw new Error("sample requires --capability <file>");
    ...
    for (const warning of lane.warnings)
        logError(`proofloop transfer-check: warning: ${warning}`);
    ...
    if (outPath) {
        ...
        (0, node_fs_1.writeFileSync)(resolved, `${serialized}\n`, "utf8");
        logError(`proofloop transfer-check: wrote sample plan to ${resolved}`);
    }
    log(serialized);
    return 0;
}
function runGateSub(options, root, log, logError) {
    const capabilityPath = strOption(options.capability);
    const browserPath = strOption(options.browser);
    if (!capabilityPath || !browserPath)
        throw new Error("gate requires --capability <file> and --browser <file>");
    ...
    for (const warning of [...capability.warnings, ...browser.warnings])
        logError(`proofloop transfer-check: warning: ${warning}`);
    ...
    for (const warning of evaluation.warnings)
        log(`proofloop transfer-check: ${warning}`);
    if (evaluation.status === "diverged") {
        log(`proofloop transfer-check: DIVERGED -- agreement ${formatPercent(evaluation.agreementRatio)}% < required ` +
        ...
        for (const disagreement of evaluation.disagreements) {
            log(`    - ${disagreement.taskId} [${disagreement.model} / ${disagreement.family}] ` +
