 ...
 * verification (the Composio proof point). PORTED from the noderoom reference
 * suite; the security-property scenarios (namespace spoof, regex prefix
 * confusion, log sabotage, prototype-pollution keys, fail-closed contract, and
 * the empty_trace fail-closed P1 regression) are preserved verbatim.
 *
 ...
 * including the adversarial angles.
 *
...
describe("tooluse CLI (compiled dist/cli.js)", () => {
...
describe("verifier: empty-trace fail-closed (adversarial P1 regression)", () => {
  ...
  // never certify a negative policy.
  const forbiddenOnly = () =>
    contractFrom({
      version: 1,
